...
- **`main` is protected by the `protect-main` ruleset** (GitHub Settings → Rules): PRs required, the `checks` CI job must pass, linear history required, force pushes and deletion blocked; the repo allows only "Rebase and merge". Direct pushes to `main` will be rejected. Repository admins can bypass — needed to merge release-please's release PRs, which never trigger CI (they're created by the Actions token); use bypass only for those.
